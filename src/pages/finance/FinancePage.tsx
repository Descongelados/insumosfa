...
import type { FacturaVenta, SalesOrder } from '../../types'
import {
...
export function FinancePage() {
  ...
  // Payment modals
  const [modal, setModal] = useState<'pago_cli' | 'pago_prov' | 'recibo' | null>(null)
  ...
  // ── Derived data ────────────────────────────────────────────────────────────
  // "Por Cobrar" = facturas con saldo pendiente (emitida / parcial / vencida)
  const porCobrar = facturasVenta.filter(f => f.saldoPendiente > 0)
  // "Pagadas" = facturas completamente cobradas
  ...
  function handlePagoCliente() {
    ...
    toast.success(`Cobro registrado: ${MXN(pagoForm.monto)}. La factura se actualizó.`)
    setModal(null)
    setPagoForm({ monto: 0, formaPago: 'Transferencia', referencia: '' })
    // After payment, switch to Pagadas tab if fully paid
            ...
            <div className="card space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    {pagadas.length} factura(s) cobradas completamente · {MXN(pagadas.reduce((a, f) => a + f.total, 0))} en total
                  </p>
                </div>
              </div>
              {pagadas.length === 0 ? (
              ...
              <select className="select" value={selFp} onChange={(e) => setSelFp(e.target.value)}>
                {facturasProveedor.filter(f => f.saldoPendiente > 0).map((f) => (
                  <option key={f.facturaProvId} value={f.facturaProvId}>
                    {f.folio} — {suppliers.find(s => s.supplierId === f.supplierId)?.razonSocial}
                  </option>
                ))}
              </select>
            </div>
