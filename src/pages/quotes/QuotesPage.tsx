...
// ── helpers ──────────────────────────────────────────────────────────────────
/** Devuelve el nombre a mostrar para una cotización */
...
/** Badge: registrado vs eventual */
function ClientBadge({ registered }: { registered: boolean }) {
  return registered ? (
...
export function QuotesPage() {
  ...
  const [filterTipo, setFilterTipo]             = useState<'todos' | 'registrado' | 'eventual'>('todos')
  ...
  const filtered = quotes.filter((qt) => {
    ...
    const isRegistered = !!qt.clienteId
    if (q && ![qt.folio, nombre].join(' ').toLowerCase().includes(q.toLowerCase())) return false
    if (filterFechaDesde && qt.fecha < filterFechaDesde) return false
    if (filterFechaHasta && qt.fecha > filterFechaHasta) return false
    if (filterClienteId && qt.clienteId !== filterClienteId) return false
    if (filterTipo === 'registrado' && !isRegistered) return false
    if (filterTipo === 'eventual' && isRegistered) return false
    return true
  })

  const hasFilters = q || filterFechaDesde || filterFechaHasta || filterClienteId || filterTipo !== 'todos'
  function clearFilters() {
    setQ(''); setFilterFechaDesde(''); setFilterFechaHasta(''); setFilterClienteId(''); setFilterTipo('todos')
  }
  ...
  async function handleSave() {
    if (form.clienteMode === 'registrado' && !form.clienteId) { toast.error('Selecciona un cliente.'); return }
    if (form.clienteMode === 'manual' && !form.clienteNombre.trim()) { toast.error('El nombre del cliente es obligatorio.'); return }
    ...
    const quote = await addQuote({
      ...
      clienteNombre:   form.clienteMode === 'manual' ? form.clienteNombre : '',
      clienteRfc:      form.clienteMode === 'manual' ? form.clienteRfc : '',
      clienteCorreo:   form.clienteMode === 'manual' ? form.clienteCorreo : '',
      clienteTelefono: form.clienteMode === 'manual' ? form.clienteTelefono : '',
  ...
  async function handleNativeShare(quote: Quote) {
    ...
    if ('share' in navigator) {
      try { await navigator.share({ title: `Cotización ${quote.folio} - ${nombre}`, text: buildShareText(quote) }); setShareOpen(false) }
    ...
    <div className="space-y-6">
      ...
      <div className="card space-y-3">
        <div className="flex items-center justify-between">
          ...
          {hasFilters && (
            <button className="text-xs text-blue-600 hover:underline" onClick={clearFilters}>Limpiar filtros</button>
          )}
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="col-span-2 md:col-span-1">
            <label className="label">Buscar</label>
            <SearchBar value={q} onChange={setQ} placeholder="Folio o cliente..." />
          </div>
          <div>
          ...
          <div>
            ...
            <select className="select" value={filterClienteId} onChange={e => setFilterClienteId(e.target.value)}>
              ...
              {clients.map(c => <option key={c.clientId} value={c.clientId}>{c.razonSocial}</option>)}
            </select>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs text-gray-500 font-medium">Tipo de cliente:</span>
          {(['todos', 'registrado', 'eventual'] as const).map(t => (
            <button
              key={t}
              onClick={() => setFilterTipo(t)}
              className={`text-xs px-3 py-1 rounded-full border font-medium transition-colors ${
                filterTipo === t
                  ? t === 'registrado' ? 'bg-blue-600 border-blue-600 text-white'
                    : t === 'eventual'  ? 'bg-amber-500 border-amber-500 text-white'
          ...
          <span className="text-xs text-gray-400 ml-2">{filtered.length} resultado{filtered.length !== 1 ? 's' : ''}</span>
        </div>
      </div>
            ...
            { key: 'folio', header: 'Folio', render: (qt) => <span className="font-mono font-semibold text-blue-700">{qt.folio}</span> },
            {
              ...
              render: (qt) => (
                <div className="flex flex-col gap-0.5">
                  ...
                  <ClientBadge registered={!!qt.clienteId} />
                </div>
              ),
            },
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() => setForm(f => ({ ...f, clienteMode: 'manual' }))}
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm font-medium transition-colors ${
                    form.clienteMode === 'manual'
            ...
            {form.clienteMode === 'registrado' ? (
              ...
              <div className="space-y-3 p-3 bg-amber-50 border border-amber-200 rounded-xl">
                <p className="text-xs text-amber-700 font-medium">Datos del cliente eventual — se guardarán en la cotización.</p>
                <div className="form-grid">
                  <div className="form-group sm:col-span-2">
                    <label className="label">Nombre / Razón Social *</label>
                    <input className="input" value={form.clienteNombre} onChange={e => setForm(f => ({ ...f, clienteNombre: e.target.value }))} placeholder="Nombre o empresa" />
                  </div>
                  <div className="form-group">
                    <label className="label">RFC</label>
                    <input className="input" maxLength={13} value={form.clienteRfc} onChange={e => setForm(f => ({ ...f, clienteRfc: e.target.value }))} placeholder="Opcional" />
                  </div>
                  <div className="form-group">
                    <label className="label">Correo</label>
                    <input type="email" className="input" value={form.clienteCorreo} onChange={e => setForm(f => ({ ...f, clienteCorreo: e.target.value }))} placeholder="Opcional" />
                  </div>
                  <div className="form-group">
                    <label className="label">Teléfono</label>
                    <input className="input" value={form.clienteTelefono} onChange={e => setForm(f => ({ ...f, clienteTelefono: e.target.value }))} placeholder="Opcional" />
                  </div>
                  <div className="form-group">
                    <label className="label">Vigencia</label>
                    <input type="date" className="input" value={form.vigencia} onChange={e => setForm(f => ({ ...f, vigencia: e.target.value }))} />
        ...
        >
          <div className="flex items-center justify-between px-4 py-3 bg-gray-900 text-white flex-shrink-0 gap-3 flex-wrap">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <ClientBadge registered={!!selQuote.clienteId} />
            </div>
        ...
        >
          <div className="space-y-3 text-sm text-gray-700">
            ...
            <div className="p-3 bg-gray-50 rounded-lg border border-gray-200 space-y-1">
              <div className="font-semibold">{resolveNombre(delTarget, clients)}</div>
              <div className="text-gray-500">Total: <Currency value={delTarget.total} /> · {delTarget.items.length} partida(s)</div>
              <div className="flex gap-2"><StatusBadge status={delTarget.estatus} /><ClientBadge registered={!!delTarget.clienteId} /></div>
            </div>
