...
const BLANK_FORM: OCForm = { supplierId: '', fechaEntregaEsperada: '', notas: '', ivaPct: 16, items: [] }
...
export function PurchasesPage() {
  ...
  useEffect(() => {
    void loadPurchases(); void loadSuppliers(); void loadProducts(); void loadInventory()
    const u1 = subPurchases(); const u2 = subSuppliers(); const u3 = subProducts(); const u4 = subInventory()
    return () => { u1(); u2(); u3(); u4() }
  }, [])
  ...
  function addItem() { setForm(f => ({ ...f, items: [...f.items, { detalleId: `ocd${Date.now()}`, productId: '', cantidad: 1, precioUnitario: 0 }] })) }
  function updateItem(idx: number, key: keyof OrdenCompraItem, val: string | number) {
    setForm(f => ({ ...f, items: f.items.map((it, i) => i !== idx ? it : { ...it, [key]: val, ...(key === 'productId' ? { precioUnitario: products.find(p => p.productId === val)?.costoPromedio ?? 0 } : {}) }) }))
  }
  ...
  function openNew() { setForm({ ...BLANK_FORM, supplierId: suppliers[0]?.supplierId ?? '' }); setModal('new') }
  function openEdit(oc: OrdenCompra) {
    setSelOC(oc)
    setForm({ supplierId: oc.supplierId, fechaEntregaEsperada: oc.fechaEntregaEsperada, notas: oc.notas, ivaPct: oc.ivaPct, items: oc.items.map(it => ({ ...it })) })
    setModal('edit')
  }
  ...
  async function handleSave() {
    ...
    toast.success(`OC ${oc.folio} creada.`); setModal(null); setForm(BLANK_FORM)
  }

  async function handleSaveEdit() {
    if (!selOC || form.items.length === 0) { toast.error('Agrega al menos un producto.'); return }
    const { total } = calcTotals(form.items, form.ivaPct)
    await updateOrdenCompra(selOC.ordenCompraId, { supplierId: form.supplierId, fechaEntregaEsperada: form.fechaEntregaEsperada, notas: form.notas, ivaPct: form.ivaPct, items: form.items, monto: total })
    toast.success(`OC ${selOC.folio} actualizada.`); setModal(null); setSelOC(null)
  }

  function handleEnviarCxP(oc: OrdenCompra) {
    void updateOrdenCompra(oc.ordenCompraId, { estatus: 'enviarPago' })
    toast.success(`OC ${oc.folio} enviada a CxP.`); setModal(null); setSelOC(null)
  }

  // Embarque automatico con el TOTAL de kg de la OC.
  // La reduccion de cantidad se gestiona en el modulo de Logistica.
  async function handleEnviarCxPLogistica(oc: OrdenCompra) {
    ...
    await addEmbarque({
      ...
      origen: '', destino: '', transportistaId: '',
      fechaProgramada: oc.fechaEntregaEsperada ?? '',
      costoFlete: 0, estatus: 'solicitado', notas: '',
      kgOriginal: totalKg,
    } as any)
    await updateOrdenCompra(oc.ordenCompraId, { estatus: 'enviarPago' })
    toast.success(`OC ${oc.folio} - Embarque creado en Logistica y OC disponible en CxP.`)
    setModal(null); setSelOC(null)
  }
  ...
  const FormModal = ({ title, onSave }: { title: string; onSave: () => void }) => (
    <Modal title={title} onClose={() => setModal(null)} size="lg"
      footer={<><button className="btn-secondary" onClick={() => setModal(null)}>Cancelar</button><button className="btn-primary" onClick={onSave}>Guardar</button></>}>
      <div className="space-y-4">
        <div className="form-grid">
          <div className="form-group"><label className="label">Proveedor *</label>
          ...
          <div className="form-group"><label className="label">Fecha Entrega Esperada</label>
        ...
        <div className="form-group"><label className="label">IVA</label>
  ...
  return (
    <div className="space-y-6">
      <div className="page-header">
        ...
        <button className="btn-primary" onClick={openNew}><Plus size={16} /> Nueva OC</button>
      </div>

      <div className="flex gap-2 mb-2">
        <button className={`btn ${tab === 'oc' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setTab('oc')}>Ordenes de Compra ({filteredOC.length})</button>
        <button className={`btn ${tab === 'sol' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setTab('sol')}>Solicitudes ({solicitudes.length})</button>
      </div>

      <div className="card">
        <div className="mb-4"><SearchBar value={q} onChange={setQ} placeholder="Buscar folio o proveedor..." /></div>
        {tab === 'oc' ? (
          <DataTable data={filteredOC} rowKey={(o) => o.ordenCompraId} columns={[
            ...
            { key: 'acc', header: '', render: (o) => (
              <div className="flex gap-1">
                ...
                {canEdit && ['borrador'].includes(o.estatus) && <button className="btn btn-secondary btn-sm" onClick={() => openEdit(o)}><Pencil size={13} /></button>}
                {o.estatus === 'confirmada' && (<>
                  <button className="btn btn-success btn-sm" onClick={() => handleEnviarCxP(o)}><CreditCard size={13} /> CxP</button>
                  <button className="btn btn-primary btn-sm" onClick={() => { void handleEnviarCxPLogistica(o) }}><Truck size={13} /> CxP y Log</button>
                </>)}
                {o.estatus === 'parcialLogistica' && <span className="text-xs font-medium text-amber-600 px-2 py-1 bg-amber-50 rounded-full border border-amber-200">Parcial</span>}
                ...
                {canDelete && ['borrador'].includes(o.estatus) && <button className="btn btn-danger btn-sm" onClick={() => { setDelOC(o); setModal('del_oc') }}><Trash2 size={13} /></button>}
                {isDirector && o.estatus === 'cerrada' && <><button className="btn btn-secondary btn-sm" onClick={() => openEdit(o)}><Pencil size={13} /></button><button className="btn btn-danger btn-sm" onClick={() => { setDelOC(o); setModal('del_oc') }}><Trash2 size={13} /></button></>}
              </div>
            )},
          ]} />
        ) : (
          <DataTable data={solicitudes} rowKey={(s) => s.solicitudId} columns={[
            { key: 'fecha', header: 'Fecha' },
            { key: 'solicitante', header: 'Solicitante' },
            { key: 'producto', header: 'Producto', render: (s) => { const p = products.find(pr => pr.productId === s.productId); return `${p?.sku ?? ''} - ${p?.descripcion ?? '-'}` }},
            ...
            { key: 'acc', header: '', render: (s) => (
              <div className="flex gap-1">
                ...
                {canDelete && ['enRevision', 'rechazada'].includes(s.estatus) && <button className="btn btn-danger btn-sm" onClick={() => { deleteSolicitud(s.solicitudId); toast.success('Solicitud eliminada.') }}><Trash2 size={13} /></button>}
              </div>
            )},
          ]} />
        )}
      ...
      {modal === 'new' && <FormModal title="Nueva Orden de Compra" onSave={handleSave} />}
      {modal === 'edit' && selOC && <FormModal title={`Editar OC ${selOC.folio}`} onSave={handleSaveEdit} />}
          ...
          <Modal title={`OC ${selOC.folio}`} onClose={() => setModal(null)} size="lg"
            footer={<div className="flex gap-2">
              ...
              {selOC.estatus === 'confirmada' && (<>
                <button className="btn-success" onClick={() => handleEnviarCxP(selOC)}><CreditCard size={15} /> Enviar a CxP</button>
                <button className="btn-primary" onClick={() => { void handleEnviarCxPLogistica(selOC) }}><Truck size={15} /> CxP y Logistica</button>
              </>)}
            </div>}>
            <div className="space-y-4">
              ...
              <div className="table-wrapper">
                <table className="table">
                  <thead><tr><th>SKU</th><th>Descripcion</th><th>Cantidad</th><th>P. Unit.</th><th>Subtotal</th></tr></thead>
                  <tbody>{selOC.items.map((it) => {
                    const p = products.find(pr => pr.productId === it.productId)
                    return <tr key={it.detalleId}><td className="font-mono text-xs">{p?.sku}</td><td>{p?.descripcion}</td><td>{it.cantidad}</td><td><Currency value={it.precioUnitario} /></td><td className="font-semibold"><Currency value={it.cantidad * it.precioUnitario} /></td></tr>
                  })}</tbody>
                </table>
              </div>
              <div className="p-3 bg-gray-50 rounded-lg space-y-1 text-sm text-right">
                ...
                <div className="text-base font-bold border-t border-gray-200 pt-1 mt-1">Total: {MXN(subtotal + iva)}</div>
              </div>
              <div>
      ...
      {modal === 'del_oc' && delOC && (
        <Modal title="Eliminar Orden de Compra" onClose={() => setModal(null)}
          ...
          <div className="space-y-2 text-sm text-gray-700">
            <p>Eliminar la OC <strong>{delOC.folio}</strong>? Esta accion no se puede deshacer.</p>
            {delOC.estatus === 'cerrada' && <p className="p-2 bg-amber-50 border border-amber-200 rounded text-amber-800 text-xs font-medium">OC cerrada. Solo el director puede eliminarla.</p>}
          </div>
        </Modal>
      )}
    </div>
