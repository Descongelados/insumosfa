...
export function PurchasesPage() {
  ...
  // ── form items UI (reutilizable en new y edit) ───────────────────────────────
        ...
        {form.items.length > 0 && (
          <div className="mt-3 p-3 bg-gray-50 rounded-lg space-y-1 text-sm text-right">
            ...
            <div className="text-gray-600">
              IVA ({form.ivaPct}%):
              <span className="font-semibold text-gray-900 ml-1">{MXN(iva)}</span>
            </div>
            <div className="text-base font-bold text-gray-900 border-t border-gray-200 pt-1 mt-1">
              Total: {MXN(total)}
            </div>
          </div>
        )}
